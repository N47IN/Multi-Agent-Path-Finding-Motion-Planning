...
while robot.step(timestep) != -1:
    ...
    if admin.getQueueLength()>0 and path==False:
        ...
        data = list(csv.reader(open("/home/navin/catkin_ws/src/Multi-Agent-Path-Finding-Motion-Planning/Webots_Scripts/path_rrt.csv")))
    ...
    if path == True:
        if np.linalg.norm(position[0:2] - goal) > 0.05 :
            ...
            try :
                ...
                df = pd.DataFrame(c)
                df.to_csv('rrt_wosmoothed_global.csv')
                steer(0,0)
