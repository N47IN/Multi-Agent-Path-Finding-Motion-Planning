...
def generate_point(point,radius):
    angle = np.pi*(np.random.random() * 2)
    x_new = max(min(point.x + radius*np.cos(angle),10), 0)
    y_new = max(min(point.y + radius*np.sin(angle),10), 0)
    xn, yn = ctop([x_new,y_new])
    print(xn,yn)
    return [x_new,y_new]
...
def RRT_explore(node_list,goal,image,radius = 1):
        ...
        curr_node = node_list[-1]
        radius = min(1.25, 0.5*np.sqrt((goal[1] - curr_node.y)**2 + (goal[0] - curr_node.x)**2))
        child = generate_point(curr_node,radius)
        tree = KDTree([(node.x,node.y) for node in node_list])
