...
def simulate(pos, obs1,obs2,filename):
    obstacles = create_obstacle(pos, obs1,obs2,SIM_TIME, NUMBER_OF_TIMESTEPS)
    ...
    control_vel = compute_velocity(
        robot_state, obstacles[:, :], v_desired)
    robot_state = update_state(robot_state, control_vel)
    robot_state_history[:4] = robot_state
    return control_vel
